...
class MessagingService(object):
    ...
    def get_unread_btw_users(self, user1, user2):
        ...
        """
        logger.info(
            'MessagingService: FETCHING THE NUMBER OF UNREAD MESSAGES BETWEEN TWO USERS')
        count = Message.objects.filter(
            sender=user2, recipient=user1, read_at=None).count()
        return count
    ...
    def get_conversations(self, user):
        ...
        """
        logger.info(
            'MessagingService: GETTING ALL CONVERSATION PARTNERS FOR SPECIFIC USER')
        print(user)
        if type(user) == int:
    ...
    def get_conversation_room(self, user1, user2):
        ...
        room = ChatRoom.objects.all().filter(
            first__in=users, second__in=users)[0]
        print("room :", room)
        return room
    ...
    def get_conversation(self, user1, user2, limit=50, reversed=False):
        ...
        conversation = Message.objects.filter(
            sender__in=users, recipient__in=users).order_by(order)
