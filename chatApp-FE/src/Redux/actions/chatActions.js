import {
  SEARCH_USER,
  CLEAR_SEARCH,
  FETCH_CONVERSATION_PARTNERS,
  FETCH_CONVERSATION,
  CLEAR_ACTIVE_CONV_MSGS,
  ACTIVE_CONV_PARTNER,
  SET_ACTIVE_CHAT,
  ADD_NEW_MESSAGE,
  ADD_NEW_MESSAGE_INACTIVE,
} from "./types";
...
export const searchUsers = (query) => (dispatch) => {
  ...
  axios
    .get(`${baseURL}uapi/users/search/?username=${query}`, {
      // params: {
      //     'query':data
      // },
      headers: {
    ...
    .then((data) => {
      dispatch({
        type: SEARCH_USER,
        payload: { searched_users: data, show_search_data: true },
      });
    })
    .catch((err) => {
      console.log(err);
...
export const fetchMyConversationPartners = () => (dispatch) => {
  ...
  axios
    .get(`${baseURL}conversation/partners`, {
      headers: {
        Authorization: `Token ${token}`,
      },
    })
    .then((response) => response.data)
    .then((data) => {
      console.log("fectch user ", data);
      //   let sortedData = data.sort(function (a, b) {
      //     var dateA = new Date(a.lastmsg_date),
      //       dateB = new Date(b.lastmsg_date);
      //     return dateB - dateA; //sort by date ascending
      //   });

      dispatch({
        type: FETCH_CONVERSATION_PARTNERS,
        payload: { conversation_partners: data },
      });
    })
    .catch((err) => {
      console.log(err);
...
export const fetchConvMessages = (data) => (dispatch) => {
  ...
  axios
    .get(`${baseURL}conversation/messages/${data.id}`, {
      headers: {
        Authorization: `Token ${token}`,
      },
    })
    ...
    .then((data) => {
      dispatch({
        ...
        payload: { active_conversation: data },
      });
    })
    .catch((err) => {
      console.log(err);
